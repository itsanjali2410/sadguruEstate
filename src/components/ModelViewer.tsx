...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  const content = useMemo<THREE.Object3D | null>(() => {
    // Handle fallback house case
    if (url === 'fallback-house') {
      const house = new THREE.Group();
      
      // House base - Clean white/light gray (increased size)
      const baseGeometry = new THREE.BoxGeometry(3, 1.5, 3);
      const baseMaterial = new THREE.MeshLambertMaterial({ color: 0xf8fafc });
      const base = new THREE.Mesh(baseGeometry, baseMaterial);
      base.position.y = 0.75;
      house.add(base);
      
      // House roof - Elegant dark slate (increased size)
      const roofGeometry = new THREE.ConeGeometry(2.2, 1.5, 4);
      const roofMaterial = new THREE.MeshLambertMaterial({ color: 0x334155 });
      const roof = new THREE.Mesh(roofGeometry, roofMaterial);
      roof.position.y = 2.25;
      roof.rotation.y = Math.PI / 4;
      house.add(roof);
      
      // Door - Clean dark gray (increased size)
      const doorGeometry = new THREE.BoxGeometry(0.5, 1.2, 0.1);
      const doorMaterial = new THREE.MeshLambertMaterial({ color: 0x475569 });
      const door = new THREE.Mesh(doorGeometry, doorMaterial);
      door.position.set(0, 0.6, 1.51);
      house.add(door);
      
      // Windows - Subtle blue tint (increased size and added more)
      const windowGeometry = new THREE.BoxGeometry(0.6, 0.6, 0.1);
      const windowMaterial = new THREE.MeshLambertMaterial({ color: 0xe2e8f0 });
      
      // Front windows
      const window1 = new THREE.Mesh(windowGeometry, windowMaterial);
      window1.position.set(-0.9, 1.5, 1.51);
      house.add(window1);
      
      const window2 = new THREE.Mesh(windowGeometry, windowMaterial);
      window2.position.set(0.9, 1.5, 1.51);
      house.add(window2);
      
      // Side windows
      const window3 = new THREE.Mesh(windowGeometry, windowMaterial);
      window3.position.set(1.51, 1.5, 0);
      house.add(window3);
      
      const window4 = new THREE.Mesh(windowGeometry, windowMaterial);
      window4.position.set(-1.51, 1.5, 0);
      house.add(window4);
    ...
    try {
      ...
      console.warn('Failed to load 3D model, using fallback house:', error);
      // Create a simple house fallback
      const house = new THREE.Group();
